...
class VoiceNFTService {
  ...
  async createVoiceNFT(data) {
    try {
      if (!data.userId) {
        throw new Error("userId가 누락되었습니다. NFT 생성에 실패했습니다.");
      }
      const newNFT = new VoiceNFT(data);
      await newNFT.save();
      return newNFT;
    } catch (error) {
      console.error("NFT 저장 중 오류 발생:", error);
      throw new Error("NFT 저장에 실패했습니다.");
    }
  }
}
